...
db: dict[int, ItemInDB] = {
    1: ItemInDB(id=1, name="MacBook", price=189900),
    2: ItemInDB(id=2, name="IPhone", price=99900),
    3: ItemInDB(id=3, name="IPad", price=69900),
    4: ItemInDB(id=4, name="AirPods", price=19900),
}
...
def get_stats(authorization: Optional[str] = Header(None)):
    ...
    return {"total_items": len(db), "total_value": sum}
    pass
...
def create_user(user: User):
    ...
    # верни созданный объект
    new_id = max(db.keys()) + 1
    new_user = UserInDB(id=new_id, **user.dict())
...
def login(login_req: LoginRequest):
    ...
    if login_req.email not in users_db:
        raise HTTPException(status_code=404, detail="User not found")
    user = users_db[login_req.email]
    if user.password != login_req.password:
        raise HTTPException(status_code=404, detail="Incorrect password")
    return {"access_token": login_req.email, "token_type": "bearer"}
    pass
...
def get_cart(authorization: Optional[str] = Header(None)):
    ...
    user = get_user_from_token(authorization)
    if not user:
        raise HTTPException(status_code=404, detail="Authorization required")
    return cart_db
    pass
...
def add_to_cart(cart_item: CartItem, authorization: Optional[str] = Header(None)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="Authorization required")
    new_id = len(cart_db) + 1
    cart_item_in_db = CartItemInDB(id=new_id, **cart_item.dict())
    cart_db[new_id] = cart_item_in_db
...
def get_cart_total(authorization: Optional[str] = Header(None)):
    ...
    for cart_item in cart_db.values():
        product = db.get(cart_item.product_id)
        if product:
